...
although it incorporates some syntactic and semantic features 
...
by the \akey{Core} and the \akey{HO-Core} background theories introduced 
...
\begin{newver}
A sort substitution $\theta$ can be applied to a term, where all sorts either in the annotation of a function $f^\tau$ or in the sort of a bound variable $(x:\tau)$ are subject to the substitution.  More precisely, the application $\theta(t)$ of a sort substitution $\theta$ to a term $t$ is defined inductively as follows, based on the grammar in Fig~\ref{fig:abstract-sorts}:
\begin{itemize}
\item $\theta(x)=x$
\item $\theta(f\ t_1 \cdots t_n) = f\ \theta(t_1) \cdots \theta(t_n)$
\item $\theta(f^\tau t_1 \cdots t_n) = f^{\theta(\tau)}\ \theta(t_1) \cdots \theta(t_n)$
\item $\theta(Q\, (x{:}\tau)\ t) = Q\, (x{:} \theta(\tau))\  \theta(t)$, for $Q\in \{\lambda, \exists, \forall\}$
\item $\theta(\akey{let}\ (x_1 = t_1) \cdots (x_n = t_n)\ \akey{in}\ t) = 
\akey{let}\ (x_1 = \theta(t_1)) \cdots (x_n = \theta(t_n))\ \akey{in}\ \theta(t)$
\item $\theta(\akey{match}\ t\ \akey{with}\ (p_1 \to t_1) \cdots (p_n \to t_n)) =$\\
  $\quad \quad \akey{match}\ \theta(t)\ \akey{with}\ (p_1 \to \theta(t_1) \cdots (p_n \to \theta(t_n))^+$
\end{itemize}
...
We write  $f{:}\tau_1\cdots\tau_n\tau \in \Sigma$
...
With this definition, it is possible to show that every closed term has 
at most one sort in a given signature $\Sigma$.
\begin{newver}
This means that we can define a total function that associates a unique sort
to each well-sorted term $t$.
We can also define a function $\pars$ that associates to each well-sorted term $t$ 
the set $\pars(t)$ of all sort parameters that explicitly occur in $t$,
that is, in a binder in $t$ or in the annotation of a function symbol in $t$.
\end{newver}
...
\new{where Case~\ref{ruleI} is the only rule applying to terms containing sort parameters, whereas the other rules only apply to terms without sort parameters}.
...
\begin{cases}
 ...
 \new{\Sigma \vdash t_i{:}\sigma_i\text{ for } i = 1,\dots, n,}\\
 \new{\text{and }f {:} \sigma_1\cdots\sigma_n\sigma \in \Sigma}
\end{cases}
\)
...
\begin{cases}
 ...
 \new{\Sigma \vdash t_i{:}\sigma_i \text{ and }a_i = \mea{\inter I}{t_i}\text{ for } i=1,\ldots,n ,}\\
%\quad \Sigma \vdash t_i : \sigma_i \text{ and } a_i = \mea{\inter I}{t_i}, \\
