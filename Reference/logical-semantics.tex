...
\pfrem{We should get rid of annotations in this chapter ($\alpha$) but
 this requires to explain somewhere that annotations do not change
 types.  Left for later.}
...
\pfrem{We need to carefully reread the above.}
...
\pfrem{Actually, datatypes instances.  Datatype is defined with sort
...
\pfrem{Some work is required in the above remark: well-foundedness is not mentioned
anywhere above, and it is rather the constraints on structure that guarantees that
the algebraic datatype exist.}
...
\pfrem{Apply is a normal (interpreted) function and is defined in the core theory.
There is therefore no dedicated rule for it here.}
...
\pfrem{Probably we have to introduce something like $Sort(\Omega)$,
 because it is the set of all monomorphic sort terms we can build with
 sort constructors.}
\begin{enumerate}
\item
