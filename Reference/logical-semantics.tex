...
the set $\pars(t, \Sigma)$ of all sort parameters that occur
in its sort, in that of one of its subterms, or in the sort of a variable in $t$.
We call a term $t$ \define{polymorphic} if $\pars(t, \Sigma)$ is non-empty.

\pfrem{Be clearer that the symbol in an ``as'' construct is not a subterm.
  ``Theorem'' for the uniqueness of typing.
  Maybe we can clean everything by removing the notation ``polymorphic term'', and relying on the ``pars'' side condition to sufficiently explain what to do.
}
\end{newver}
