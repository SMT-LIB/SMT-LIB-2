...
\pfrem{I might happen that we never use $\tau$ at all and the
 figure could be simplified to $\tau$ only.  Actually, to avoid
 corrupting the whole chapter, we might just extend the definition
 of $\tau$ and forget about $\tau$.}
...
\pfrem{On Nov 27, we discussed the need for quantifiers over sort parameters.
  Since only one quantifier is alowed, mandatory on all free parameters, and at
  the top symbol, we can consider that top-level terms are closed on all parameters (PCTerms).
  We have to remove universal sort binder from binders, add parameters in signatures.
  Well-sortedness might not change much.  However we will have to be careful with interpretations.
  }
...
use also \define{polymorphic sorts}.
These are defined similarly to sorts in the example above 
except that they can be built also over 
...
In the following, we say just use ``sort'' to refer to polymorphic sorts
...
For all $n \geq 0$,
distinct variables $x_1, \ldots, x_n \in \va$,
sorts $\tau_1, \ldots, \tau_n$, and
sort parameters $u_1, \ldots, u_n$,


\begin{itemize}
\item
the prefix construct
\ $\lambda\,x_1{:}\tau_1\:\cdots\:x_n{:}\tau_n\ \_$ \ is 
a \define{function abstraction binder}\index{binder!lambda binder}
for $x_1, \ldots, x_n$;

\item
the prefix construct
\ $\exists\,x_1{:}\tau_1\:\cdots\:x_n{:}\tau_n\ \_$ \ is 
a \define{sorted existential binder (or existential quantifier)}\index{binder!existential binder}
for $x_1, \ldots, x_n$;

\item
the prefix construct
\ $\forall\,x_1{:}\tau_1\:\cdots\:x_n{:}\tau_n\ \_$ \ is 
...
\item
the prefix construct
\ $\forall\,u_1\:\cdots\:u_n\ \_$ \ is 
a \define{universal sort binder}\index{binder!universal sort binder}
for $u_1, \ldots, u_n$.
\end{itemize}

Occurrences of variables in terms or parameters in types are defined to be \define{free}\index{free} or
\define{bound}\index{bound} as in the case of the concrete syntax;
the scope of each bound variable/parameter is defined similarly as well
...
\pfrem{Extend concept of closed term to include parameters}
...
sorts $\tau_1,\ldots,\tau_n,\tau \in \sortt{\sorts \Sigma}$,
...
a pair $(f, \tau_1\cdots\tau_n\tau)$ in $\fu \times \sortt{\Sigma}^+$,
which we write as $f{:}\tau_1\cdots\tau_n\tau$.
A \define{sorted variable} is 
a pair $(x,\tau)$ in $\va \times \sortt{\Sigma}$,
...
for some sort $\tau \in \sortt{\Sigma}$.
In that case, 
...
can be rewritten as 
\[
 \exists\: x_1{:}\tau_1 \ldots x_n{:}\tau_n\: \varphi\ .
\]
...
which we call the \define{domain} of $\tau$ in $\str A$,
with $A = \bigcup_{\tau \in \sorts\Sigma} \tau^{\str A}$;\endnote{
...
\item
$\mea{\inter I}{x} = v(x{:}\tau)$ 
\quad if $\inter I = (\str A, v)$  

\item
$\mea{\inter I}{\hat{f}\;t_1\;\ldots\;t_n} = 
 (f{:}\tau_1\cdots\tau_n\tau)^{\str A}(a_1,\ldots,a_n)
$ 
\quad if \  
\(
\begin{cases}
 \inter I = (\str A, v) \text{ with signature } \Sigma,\\
 \hat{f} = f \text{ or } \hat{f} = f^\tau,\\
 \text{for } i=1,\ldots,n \\
 \quad \Sigma \vdash t_i : \tau_i \text{ and } a_i = \mea{\inter I}{t_i}
\end{cases}
\)
...
\begin{cases}
 ...
 \text{for } i=1,\ldots,n \\
 \quad \Sigma \vdash t_i : \tau_i \text{ and } a_i = \mea{\inter I}{t_i}, \\
 \inter{I}' = {\inter I}[x_1{:}\tau_1\mapsto a_1,\;\ldots,\;x_n{:}\tau_n\mapsto a_n]
\end{cases}
\)

\item
$\mea{\inter I}{\exists\, x_1{:}\tau_1\, \cdots\, x_n{:}\tau_n\; t} = \true$ 
...
\begin{cases}
 \inter{I}'= {\inter I}[x_1{:}\tau_1\mapsto a_1,\;\ldots,\;x_n{:}\tau_n\mapsto a_n],\\
 (a_1,\ldots,a_n) \in \tau_1^{\str A} \times \cdots \times \tau_n^{\str A},\\
 \inter I = (\str A, v),\\
\end{cases}
\)

\item
$\mea{\inter I}{\forall\, x_1{:}\tau_1\, \cdots\, x_n{:}\tau_n\; t} = \true$ 
iff \ $\mea{\inter I'}{t} = \true$ 
for all $\inter I'$
