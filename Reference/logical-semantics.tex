...
\pfrem{People are confused with prefix as in concrete syntax, $s t_1 ...t_n$,
without parentheses, $s(t_1,\dots t_n)$.}
...
then the expression
\[
  \fconstructa{\akey{Int}}{\akey{List}(\akey{Array}(\akey{Int}, \akey{List}(\akey{Real})))}
\] 
and all of its subexpressions are monomorphic sorts.  The
constructor for function sorts $\fconstruct$ is used in an infix way to improve readability.
...
\pfrem{I had the remark that what makes this chapter complicated is
data types.  Maybe for SMT3 we can leave them aside and have a full
section just on data types as a fix?}
...
\begin{itemize}
\item
a set $\sorts{\Sigma} \subseteq \so$ of \define{sort} symbols containing $\bool$ and $\fconstruct$;
...
\pfrem{It is not too clear what is meant here: for me, it is the top symbol of the sort 
$\tau$ above}
...
% and algebraic data types (described in Section~\ref{sec:new-symbols}),
...
% and algebraic data type declarations
...
of the data type appear in one of the patterns.
...
of those variables.
In both cases, the preconditions ensure that $\delta$ is a data type, 
not just any sort, by requiring it to have a non-empty set of constructors.
\end{remark}
\pfrem{Check the above, I have a vague memory that we recently changed the rules
for match in the concrete syntax}
...
for every $a \in G$ \pfrem{element in $G$} and monomorphic sort $\sigma \in \sorts\Sigma$ 
...
where $S$ collects the sorts of $\sortt{\Sigma}$ that are not data types;
\pfrem{Actually, data types instances.  Datatype is defined with sort
constructors, here we mean all possible sorts built with these sort constructors.
See previous remark about definition of a data type}
...
guarantee that sorts with constructors indeed denote algebraic data types
...
have the same data type constructors, selectors and testers, and they agree 
...
\emph{whose set of sort symbols that are not data types
...
\item
A function symbol of $\widehat\Omega$ is a data type constructor/selector/tester 
...
and are not data type constructors, selectors or testers.\footnote{%
...
Figure~\ref{fig:logic-declaration}.  Again, the concrete
syntax allows more logic attributes, that do not matter here.
\pfrem{We should talk about extensions.}
...
Recall that algebraic data types are not defined in theories but directly 
...
For example, in a parametric list data type $\delta$ 
