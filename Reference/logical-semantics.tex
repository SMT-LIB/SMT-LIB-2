...
\ctrem{We should check that this type system assigns a unique sort to a well-sorted term. 
And it is consistent with global quantification of sort parameters.
}
...
the set $\pars(t, \Sigma)$ of all parameters that occur
in its sort, in that of one of its subterms, or in the sort of a variable in $t$.
We call a term \define{polymorphic} if $\pars(t, \Sigma)$ is non-empty.
\end{newver}
