...
\pfrem{There are still $\alpha$ attributes in theory and logic declaration
To CT: maybe we should remove useless things from theory and logic.}
...
then the expression
\[
  \fconstruct(\akey{Int},\akey{List}(\akey{Array}(\akey{Int}, \akey{List}(\akey{Real}))))
\] 
...
the expression
\[
  \fconstruct(u_1, \fconstruct(u_2,\akey{List}(\akey{Array}(u_1, \akey{List}(u_2)))))
\]
