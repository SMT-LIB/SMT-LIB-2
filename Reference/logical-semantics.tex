...
A sort substitution $\theta$ can be applied to a term, where all sorts either in the annotation of a function $f^\tau$ or in the type of a bound variable $(x:\tau)$ are subject to the substitution.  More precisely, the application $\theta(t)$ of a sort substitution $\theta$ to a term $t$ is defined inductively as follows, based on the grammar in Fig~\ref{fig:abstract-sorts}:
\begin{itemize}
\item $\theta(x)=x$
...
\pfrem{We should prove that $\theta(t)$ is well-typed if $t$ is.}
...
\begin{newver}
This means that we can define a total function that associates 
with each well-sorted term $t$ a unique sort.
We can also define a function $\pars$ that associates to each well-sorted term $t$ the set $\pars(t)$ of all sort parameters that occur
in its sort, in that of one of its subterms, or in the sort of a variable in $t$.  \pfrem{Say something about the annotations.}
\end{newver}
