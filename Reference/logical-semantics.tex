...
\pfrem{Revise}
The application of a sort substitution $\theta$ to a term $t$ is defined inductively as follows, based on the grammar in Fig~\ref{TODO 5.2}
\begin{itemize}
\item $\theta(x)=x$
...
We can also define a function $\pars$ that associates to each well-sorted term $t$ the set $\pars(t, \Sigma)$ of all sort parameters that occur
in its sort, in that of one of its subterms, or in the sort of a variable in $t$.
\end{newver}
