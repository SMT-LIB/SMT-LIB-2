...
although it incorporates some syntactic features of higher-order logics:
...
the logic's type system has no function (\emph{arrow}) types,
...
by the \akey{Core} background theory introduced 
in Subection~\ref{sec:theory-examples}.
...
The formal semantics of concrete SMT-LIB expressions is then given by means of 
a translation into this abstract syntax.
...
the set $\sortt S$ of all \define{sorts}\index{sort} over $S$ 
(with respect to $\ar$)
...
$\sigma_1, \ldots, \sigma_n$ are sorts,
then the term
$s\,\sigma_1\,\cdots\,\sigma_n\:$ is a sort.
\end{enumerate}
...
and $\akey{List}$ and $\akey{Array}$ are sort symbols of respective arity 1 and 2,
then the expression
$\akey{List}\ (\akey{Array}\ \akey{Int}\ (\akey{List}\ \akey{Real}))$ 
and all of its subexpressions are sorts.

Function symbol declarations in theory declarations (defined later)
use also \define{parametric sorts}.
These are defined similarly to sorts above except that
they can be built also over 
a further set $\mathcal{U}$ of \define{sort parameters},
used like sort symbols of arity 0.
Similarly to the example above,
if $u_1, u_2$ are elements of $\mathcal{U}$,
the expression $\akey{List}\ (\akey{Array}\ u_1\ (\akey{List}\ u_2))$ 
and all of its subexpressions are parametric sorts.

An abstract syntax for sorts $\sigma$ and parametric sorts $\tau$,
which ignores arity constraints for simplicity, 
is provided in Figure~\ref{fig:abstract-sorts}.
Note that every sort is a parametric sort, but not vice versa.
Also note that parametric sorts are used only in theory declarations;
they are not part of the SMT-LIB logic proper.
In the following, we say ``sort'' to refer exclusively to non-parametric sorts.
...
%
The logic considers, in fact, only \emph{well-sorted terms},
...
For all $n \geq 0$,
distinct variables $x_1, \ldots, x_n \in \va$, and 
sorts $\sigma_1, \ldots, \sigma_n$,

\begin{itemize}
\item
the prefix construct
\ $\exists\,x_1{:}\sigma_1\:\cdots\:x_n{:}\sigma_n\ \_$ \ is 
a \define{sorted existential binder (or existential quantifier)}\index{binder!existential binder}
for $x_1, \ldots, x_n$;

\item
the prefix construct
\ $\forall\,x_1{:}\sigma_1\:\cdots\:x_n{:}\sigma_n\ \_$ \ is 
...
% as removed for now:
%\trem{Add formal definition of bound variables and \define{closed} term}
...
Function symbols themselves may be annotated with a sort, as in $f^\sigma$.
...
\item
a partial mapping from $\va$ to $\sortt{\sorts\Sigma}$,
assigning a sort to some of the variables in $\va$;%
%so that there are infinitely many variables of sort $\sigma$ 
%for each $\sigma \in \sortt{\sorts\Sigma}$;
\footnote{%
...
$R$ from $\funs{\Sigma}$ to $\sortt{\sorts \Sigma}^+$,
...
%
%\item
%$(\eqs,\, \sigma\sigma\,\bool) \in R$ for all $\sigma \in \sortt{\sorts \Sigma}$;

\item
for all constructors $c \in \cons{\Sigma}$, 
selectors $g_1 \cdots g_n = \mathrm{sel}_\Sigma(c)$, and
sorts $\sigma_1,\ldots,\sigma_n,\sigma \in \sortt{\sorts \Sigma}$,
if $(c, \sigma_1\cdots\sigma_n\sigma) \in R$
then $(g_i,\sigma\sigma_i) \in R$ for all $i=1,\ldots,n$;

\item
for all constructors $c \in \cons{\Sigma}$ and 
testers $p = \mathrm{tes}_\Sigma(c)$,
if $(c, \sigma_1\cdots\sigma_n\sigma) \in R$
then $(p,\sigma\bool) \in R$;

\item
there is no constructor $c \in \cons{\Sigma}$ such that
$(c, \bar{\sigma}_1\sigma), (c, \bar{\sigma}_2\sigma) \in R$ 
for distinct $\bar{\sigma}_1$ and $\bar{\sigma}_2$.\endnote{
...
A sort in $\Sigma$ is an \define{algebraic data type}
...
In the following, we will write $\sortt{\Sigma}$ as an abbreviation 
of $\sortt{\sorts \Sigma}$.
...
a pair $(f, \sigma_1\cdots\sigma_n\sigma)$ in $\fu \times \sortt{\Sigma}^+$,
which we write as $f{:}\sigma_1\cdots\sigma_n\sigma$.
A \define{sorted variable} is 
a pair $(x,\sigma)$ in $\va \times \sortt{\Sigma}$,
which we write as $x{:}\sigma$.
We write 
$f{:}\sigma_1\cdots\sigma_n\sigma \in \Sigma$ and
$x{:}\sigma \in \Sigma$
to denote that $f$ has rank $\sigma_1\cdots\sigma_n\sigma$ in $\Sigma$ and 
$x$ has sort $\sigma$ in $\Sigma$.

We also consider signatures that differ from a given signature $\Sigma$
only the sort they assign to variables.
We will also consider signatures that conservatively expand a given signature $\Sigma$
...
$x{:}\sigma \in \Sigma$ iff $x{:}\sigma \in \Omega$;
for all $f \in \fu$ and $\bar{\sigma} \in \sortt{\Sigma}^+$,
if $f{:}\bar{\sigma} \in \Sigma$ then $f{:}\bar{\sigma} \in \Omega$.
...
$(\akey{Array}\:\sigma_1\:\sigma_2)\:\sigma_1\:\sigma_2$ 
for all sorts $\sigma_1, \sigma_2$ in a theory signature. 
%
...
$\bar{\sigma}\sigma_1$ and $\bar{\sigma}\sigma_2$ 
where $\bar{\sigma}$ is a sequence of sorts and 
$\sigma_1$ and $\sigma_2$ are different sorts.
...
$\bar{\sigma}\sigma_1, \ldots, \bar{\sigma}\sigma_n$,
a term of the form $f\, \bar{t}$,
where the terms $\bar t$ have sorts $\bar \sigma$, can be given 
a unique sort only if $f$ is annotated with one of the result sorts
$\sigma_1, \ldots, \sigma_n$,
that is, only if it is written as $f^{\sigma_i}\bar{t}$ 
...
In more detail, the letter $\sigma$ (possibly primed or with subscripts) 
denotes sorts in $\sortt \Sigma$,
the letter $k$ denotes a natural number.
%
%the notation $x : \sigma \in \Sigma$ means that 
%$\Sigma$ maps variable $x$ to sort $\sigma$.
%The notation $f:\sigma_1 \cdots \sigma_{k+1} \in \Sigma$ means that 
%$f$ is an element of $\funs{\Sigma} \cup \{\eqs\}$ 
%and 
%has rank $\sigma_1 \cdots \sigma_{k+1}$ in $\Sigma$. 
Expressions of the form $\Sigma[x_1:\sigma_1,\: \ldots,\: x_n:\sigma_n]$ 
denotes the signature 
that maps $x_i$ to sort $\sigma_i$ for $i=1,\ldots,n$, and 
...
which are triples of the form $\Sigma \vdash t : \sigma$.
...
if $\Sigma \vdash t : \sigma$ is derivable 
by the sorting rules in Figure~\ref{fig:well-sorted-terms}
for some sort $\sigma \in \sortt{\Sigma}$.
In that case, 
we say that \define{$t$ has, or is of, sort $\sigma$}.
\qed
\end{definition}
...
most one sort.\endnote{
  ...
  $(f^\sigma\; t_1\; \cdots\; t_k)\ \alpha^*$ with annotated function
  symbols $f^\sigma$, without the second conjunct of the rule's side
  condition.  This would allow formation of terms with annotated
  function symbols $f^\sigma$, even when $f$ lacked two ranks of the
  forms $\sigma_1 \cdots \sigma_k\sigma$ and $\sigma_1 \cdots
  \sigma_k\sigma'$, for distinct $\sigma$ and $\sigma'$.  The
...
$x_1, \ldots, x_n$ of respective sort $\sigma_1, \ldots, \sigma_n$,
can be rewritten as 
\[
...
with $A = \bigcup_{\sigma \in \sorts\Sigma} \sigma^{\str A}$;\endnote{
...
as an element $(f{:}\sigma)^{\str A}$ of  $\sigma^{\str A}$;

\item
each $f{:}\sigma_1\cdots\sigma_n\sigma \in \Sigma$ with $n > 0$ 
as a total function 
$(f{:}\sigma_1\cdots\sigma_n\sigma)^{\str A}$ 
from $\sigma_1^{\str A} \times \cdots \times \sigma_n^{\str A}$ to $\sigma^{\str A}$.
\end{itemize}
...
a constant symbol $a$ of rank $\sigma$
for every $a \in G$ and sort $\sigma \in \sorts\Sigma$ 
such that $a \in \sigma^{\str A}$.
Then, $\str A$ is an \define{absolutely free structure (with generators $G$)}
if 
\begin{itemize}
\item
for all $\sigma \in \sorts\Sigma$, $\sigma^{\str A}$ is the set 
...
%CT unnecessary
%\item
%${\eqs}{:}\sigma\sigma\,\bool$ is interpreted as the identity predicate over $\sigma^{\str A}$ \footnote{%
%That is,
%for all $\sigma \in \sortt{\Sigma}$ and all $a,b \in \sigma^{\str A}$,\ 
%${\eqs}^{\str A}(a,b) = \true$ iff $a$ is the same as $b$.
%};

%CT previous, incomplete restrictions on constructors
%\item
%for all (ranked) constructors $c{:}\sigma_1\cdots\sigma_n\sigma \in \Sigma$ with $n>0$,
%$c^{\str A}$ is injective;
%
%\item
%for all sorts $\sigma$ with a set $\{c_1, \ldots, c_n\}$ of $n>0$ constructors,
%the ranges of $c_1^{\str A}, \ldots, c_n^{\str A}$ form a partition of $\sigma^{\str A}$;
...
This means in particular that if $g{:}\sigma\sigma_i \in \Sigma$ is a selector,
...
other than it belongs to $\sigma_i^{\str A}$.
...
\begin{enumerate}
\item
for all $\sigma \in \sortt{\Sigma}$ and $a \in \sigma^{\str A}$,
\[
 h(a) \in  \sigma^{\str B}\ ;
\]
\item
...
%Because each $a_i$ need not be in $\sigma_i^{\str A}$ 
%where $\sigma_i$ is the sort of $x_i$ in $\Sigma$.
%}
...
%if $\exists x{:}\sigma \varphi$ is a formula of some signature $\Sigma$,
%its subformula $\varphi$ is, for typing \emph{and} semantic purposes, 
%a formula of signature $\Sigma[x{:}\sigma]$.
%}
...
For every well-sorted term $t$ of sort $\sigma$ with respect to $\Sigma$,
$\mea{\inter I}{t}$ is defined recursively as follows.

\begin{enumerate}
\item
$\mea{\inter I}{u\;\alpha_1\;\cdots\;\alpha_n} = \mea{\inter I}{u}$
...
for all $f \in \fu$ and $\bar{\sigma} \in \sortt{\Sigma}^+$,
$f{:}\bar{\sigma} \in \Sigma$ iff 
$f{:}\bar{\sigma} \in \Sigma_1$ or
$f{:}\bar{\sigma} \in \Sigma_2$.
...
$\bar{\sigma} \in (\sorts{\widehat\Omega})^+$, 
...
\item \label{decl-i}
$T$ contains a declaration of the form $f\;\bar{\sigma}\;\bar{\alpha}$, or

\item \label{decl-ii}
$T$ contains a declaration of the form $\Pi\;\bar{u}\;(f\;\bar{\tau}\;\bar{\alpha})$ and
$\bar{\sigma}$ is an instance of $\bar \tau$.
\end{enumerate}
...
$\str A$ is such that,
for all $\sigma \in \sorts{\Sigma}$ and 
