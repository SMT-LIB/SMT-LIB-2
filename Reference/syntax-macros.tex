...
\newcommand{\fconstruct}{\akey{Fun}}
\newcommand{\fconstructa}[2]{\akey{Fun}(#1, #2)}
...
\begin{tabular}{ll}
  ...
  {\Sigma \vdash (f\; t_1\; \cdots\; t_k)\ \alpha^*: \tau}
  &
...
\begin{tabular}{ll}
  ...
  {\Sigma \vdash (\lambda (x {:} \tau_1)\; t_2) \alpha^*: \fconstructa{\tau_1}{\tau_2}}
  &
  \end{tabular}
  \bigskip
...
\begin{tabular}{ll}
  ...
  {\Sigma \vdash (f^\tau\; t_1\; \cdots\; t_k)\ \alpha^*: \tau}
  &
  if 
  \(
  \begin{cases}
  f{:}\tau_1 \cdots \tau_k\tau \in \Sigma, \\
  f{:}\tau_1 \cdots \tau_k\tau' \in \Sigma \ \text{ for some } \tau' \neq \tau
  \end{cases}
  \)
  \end{tabular}
  \bigskip
...
{\Sigma \vdash (Q x{:}\tau\; t)\ \alpha^* : \bool}
&
...
{\Sigma \vdash (\akey{let}\ x_1 = t_1\;\cdots\; x_{k+1} = t_{k+1}\ \akey{in}\ t)\ \alpha^* : \tau}
&
...
{\Sigma \vdash (\akey{match}\ t\ \akey{with}\ c_1\:\bar x_1 \to t_1 \;\cdots\; c_{k+1}\:\bar x_{k+1} \to t_{k+1})\ \alpha^* : \tau}
&
if 
\(
\begin{cases}
...
{\Sigma \vdash (\akey{match}\ t\ \akey{with}\ p_1 \to t_1 \;\cdots\; p_n \to t_n)\ \alpha^* : \tau}
&
if 
\(
\begin{cases}
