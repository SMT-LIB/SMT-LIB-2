...
\paragraph{Release: 2026-03-27}
\begin{itemize}
\item Clarified a number of points on polymorphic constants and functions, in particular
with respect to the \ter{let} binder in the language and maps in theory HO-Core.
\end{itemize}

\paragraph{Release: 2025-07-07}
\begin{itemize}
\item Clarified that quantifier patterns of the form \expr{:pattern ()} are allowed.
\item Clarified the interactions between the \attr{:print-success} option, the \expr{exit} command, and commands returning values.
\item Clarified that every formula returned by the \expr{get-assertions} command must include \attr{named} and \attr{pattern} annotations, if present in the initially asserted formula.
\item Fixed several typos and minor errors.
\item Clarified difference between sort parameter and uninterpreted sort.
\end{itemize}

\paragraph{Release: 2025-04-09}
\begin{itemize}
\item Replaced \ter{\_} with \ter{@} as the map application operator in theory HO-Core.
\end{itemize}

\paragraph{Release: 2025-02-05}
\begin{itemize}
...
function abstractions and a symbol for function application, \ter{@},
...
the binary sort constructor \ter{->} for map sorts.
...
The reserved word \ter{\_} can now also be used (unambiguously) as a wildcard symbol in \ter{match} patterns.
...
the commands \expr{get-unsat-assumptions} and \expr{get-unsat-core} has been
...
\item
The standard option \attr{expand-definitions} has been removed
...
when we want to relate the number of repetitions in several occurrences of the latter operators.
