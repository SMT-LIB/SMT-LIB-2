...
in the current signature.}\endnote{\new{SMT-LIB 2.7 somehow misnames sort
parameters and variables.  In particular the \expr{par} constructs
actually introduces sort variables.  We have adopted a pragmatic approach
to this issue, not renaming the \expr{par} construct, for backward
compatibility.}}
\smallskip
...
where the $u_i$'s are symbols used as \define{sort variables}, and $\tau$ may contain sort
variables $u_1$, \dots, $u_n$\new{, but no previously declared sort parameters}.
...
where $\tau$ contains $s$.
\begin{newver}
These sort variables $u_i$'s shadow any previously user-declared sort symbol
or parameter with the same name.
However, they do not shadow theory sort symbols, analogously to bound variables
versus declared theory symbols.  The command reports an error if a theory sort
...
%\pfrem{sort variables in datatypes must come from the par list.}
...
\endnote{\new{Using sort variables as opposed to globally declared sort parameter
...
% the order of the sort variables in par
...
\footnote{\new{No previously declared sort parameters that is not shadowed by a $u_i$ can be used.}}%
...
%
\new{
\item[\expr{(define-const $f$ $t$)}] has the same effect as
\expr{(define-fun $f$ () $t$)}.
\smallskip
}
...
\begin{remark}
  ...
  \end{lstlisting}
  states that every sort has a domain of size one.  The negation of the assertion
  above would invole only one sort.  Properly negating this assertion then requires
  the introduction of a fresh sort as in
  \begin{lstlisting}
  (declare-sort U 0)
  ...
  \end{lstlisting}
  state that every domain must be of size at least two.
  This means that special care must be taken in checking, e.g., the
  validity of an implication \expr{(=> $p$ $q$)} by asserting $p$ and
  the negation of $q$, and checking for satisfiability, when polymorphic
  sorts are involved.
\end{remark}
\end{newver}
