...
%  yshift=2cm, 		% move 2cm in y
 ...
 \node[state,    	% layout (defined above)
  ...
  anchor=center] (SAT) 	% position relative to the center of the 'box'
 {\textbf{Sat mode}};
...
These channels may be set with the \ter{set-option} command (see Section~\ref{sec:options} below).  
By default they are the standard output and standard error channels, 
respectively.

For most commands, once a solver completes its processing, 
...
\noindent
By default, the \attr{print-success} option is set to \ter{false}.
When the \attr{print-success} option is set to \ter{true}, the value \ter{success} is the default
response for a successful execution of a supported command.  
...
The standard \attr{error-behavior} flag can be used 
...
Solver options may be set using the \ter{set-option} command, and their current
values can be obtained using the \ter{get-option} command.
...
It is understood that if a solver does not support one of the non-required 
standard options below, it behaves as if that option was permanently
set to its default value.
Specifically, the output of the \ter{get-option} command for a non-required
standard option should be the default value, whereas the output of
the \ter{set-option} command for that option should be \ter{unsupported}.
For unsupported non-standard options, the output of both \ter{get-option}
and \ter{set-option} should be \ter{unsupported}.
...
If the option is one of the standard ones it should also leave it unchanged 
...
The first command below allows the declaration of sort parameters.
...
any of $u_1$, \dots, $u_n$, but no global sort parameters.
...
the terms $\tau_i$ can contain only the sort parameters in the list $u_1, \ldots, u_k$.%
\footnote{%
...
\begin{remark}
  Notice the difference between a sort parameter, as in this example:
\begin{lstlisting}
(declare-sort-parameter A)
...
\end{lstlisting}
In the first case, the second command asserts that \emph{all sorts} constructible with the available sort constructors have a singleton domain.  In the second case instead, the assertion restricts only sort \ter{U} to have a singleton domain.
\end{remark}
...
Each $f_i$ is a formula syntactically identical, including occurrences
of annotations with the \attr{named} and \attr{pattern} attributes,
to one of the formulas entered with an \ter{assert} command and 
currently in the context.
The printed formulas may differ in the use of whitespace.
Annotations besides \attr{named} and \attr{pattern} need not be included.
...
is satisfiable in the extension of the current context with all the current 
...
not in the current signature only if they are abstract values.
Moreover, each occurrence of an abstract value $a$ of sort $\sigma$ 
...
\begin{center}
  \expr{(define-fun-rec f ((x Int)) Int (ite (< x 0) (f x) (ite (= x 0) 1 (* x (f (- x 1))))))}
\end{center}
...
For an unsupported non-standard option the command outputs \ter{unsupported}.  
\end{description}
...
There is only a small number of standard \ter{set-info} flags,
which are described below together with their possible values.
These flags are used in particular in the official SMT-LIB benchmarks
...
\expr{(set-info :status $v$)} in the script, if $v$ is either \ter{sat} or \ter{unsat}.  If $v$ is unknown, there is no expectation on the value of the check command.\endnote{
