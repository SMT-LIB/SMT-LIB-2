...
\new{
adds sort parameter $s$ to the current signature.
The command reports an error if $s$ is a sort symbol or parameter already present 
in the current signature.
}\endnote{%
\new{
SMT-LIB 2.7 actually misnames sort parameters and variables.
In particular the \expr{par} constructs actually introduces sort variables.
We have adopted a pragmatic approach to this issue, 
not renaming the \expr{par} construct, for backward compatibility.
}
}
\smallskip
...
\endnote{
\new{
Using sort variables as opposed to globally declared sort parameter
...
\footnote{%
\new{
No previously declared sort parameters that is not shadowed by a $u_i$ can be used.
}
}
...
The negation of the assertion above would involve only one sort.
Properly negating this assertion then requires the introduction 
of a fresh sort symbol, as in
\begin{lstlisting}
(declare-sort U 0)
...
\end{lstlisting}
have the effect of stating that \emph{every} sort has a domain of size 
at least two.
In general, this means that, when polymorphic sorts are involved,
special care must be taken in checking, for example, the validity 
